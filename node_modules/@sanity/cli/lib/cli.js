...
module.exports = async function runCli(cliRoot) {
  ...
  _dotenv.default.config({
    path: _path.default.join(workDir, `.env.${env}`)
  });
  ...
  cliRunner.runCommand(args.groupOrCommand, args, options).catch(err => {
    ...
    console.error(`\n${error.stack ? (0, _neatStack.default)(err) : error}`);
...
function resolveRootDir(cwd) {
  ...
  try {
    ...
    console.warn(_chalk.default.red(['Error occurred trying to resolve project root:', err.message].join('\n')));
    process.exit(1);
  }
  return false;
}
