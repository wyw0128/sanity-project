...
async function upgradeDependencies(args, context) {
  ...
  const newManifest = nonPinned.reduce((target, mod) => {
    ...
    if (oldManifest.dependencies && oldManifest.dependencies[mod.name]) {
      target.dependencies[mod.name] = typeof mod.latestInRange === 'undefined' ?
    ...
    if (oldManifest.devDependencies && oldManifest.devDependencies[mod.name]) {
      target.devDependencies[mod.name] = typeof mod.latestInRange === 'undefined' ?
      // Keep as-is if we can't resolve within range/tag
      oldManifest.dependencies[mod.name] :
      // Store latest within range, or just the tag name if using that
      savedVersion;
  ...
  await Promise.all(nonPinned.map(mod => deleteIfNotSymlink(_path.default.join(context.workDir, 'node_modules', mod.name.replace(/\//g, _path.default.sep)))));
